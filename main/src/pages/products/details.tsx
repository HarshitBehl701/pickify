...
function ProductDetail() {
  ...
  const [selectedImage, setSelectedImage] = useState<string>(
    "/asset/mainAssets/logos/logo.png"
  );
  ...
  useEffect(() => {
    ...
    {
      ;(async() =>{
        await axios.post('/api/products/add_view',{product_id:productData.id});
      })()
    }
  },[productData]);
  ...
  const  handlePlaceOrder =  useCallback(async  () =>  {
    if(productData && isCurrentProductInCart  &&  cartId)
    {
      try {
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const  response  =  await axios.post("/api/orders/place_order",{cart_id:cartId},{withCredentials:true});
  ...
  const  [isCurrentProductInOrder,setIsCurrentProductInOrder] = useState<boolean>(false);
  const  [isCommentLocked,setIsCommentLocked] = useState<boolean>(true);
  const  [orderId,setOrderId] = useState<number | null>(null);
  const  [userRating,setUserRating] = useState<number | null>(null);
  
  useEffect(() => {
    if(productData && userOrders)
    {
  ...
  const  handleAddRating  = useCallback(async(rating:number) => {
    if(orderId)
    {
      try {
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const  response  =  await axios.post("/api/orders/add_order_rating",{order_id:orderId,rating:rating},{withCredentials:true});
  ...
  return (
    <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div>
            <div className="border  w-full h-96 rounded-lg overflow-hidden">
              <Image
                src={
                  selectedImage == "/asset/mainAssets/logos/logo.png"
                    ? selectedImage
          ...
          <div className="space-y-4">
            ...
            {[...Array(5)].map((_, i) => {
              return <Star key={i} size={20} fill={userRating && i < userRating ? '#FACC15' :  '#6B7280'} stroke={userRating && i < userRating ? '#FACC15' :  '#6B7280'} className="cursor-pointer" onClick={() => handleAddRating(i + 1)}/>
            })}
            </div>
            {/* Quantity Selector */}
            ...
            <div className="flex gap-4 flex-wrap">
              {isCurrentProductInCart && <button className="bg-blue-500 cursor-pointer text-white px-6 py-3 rounded-lg font-semibold hover:bg-blue-500" onClick={handlePlaceOrder}>Place  Order</button>}
              <button
      ...
      {/* Comments */}
      {productData &&  <Comments product_id={productData?.id} isCommentAllowed={!isCommentLocked} order_id={orderId} />}
