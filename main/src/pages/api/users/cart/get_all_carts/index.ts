...
async function handler(req: ICustomApiRequest, res: NextApiResponse) {
    try {
        ...
        if (!(rows as IUserCartResponse[]).length) {
            return res.status(404).json(responseStructure(false, "No cart items found"));
        }
        ...
        return res.status(200).json(responseStructure(true, "Successfully fetched cart", cart));
    } catch (error) {
