...
export async function POST(req: NextRequest) {
    try {
        ...
        const [rows] = await mysqlDb.execute(`SELECT 
            ...
            FROM \`categories\` category LEFT JOIN \`sub_categories\` sub_category 
        ...
        return NextResponse.json(responseStructure(true, "Successfully fetched all categories and sub-categories", categoriesAndSubCategories));
    } catch (error) {
