...
const EditSubCategory = ({ subCategory }: { subCategory: ISubCategory }) => {
  ...
  useEffect(() => {
    if (Object.keys(formData).length === 0 && subCategory) {
      setFormData({
        sub_category_id: subCategory.id,
        name: subCategory.name,
        is_active: subCategory.is_active as 0 | 1,
      });
    }
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        await axios.post("/api/main/sub_category/manage_sub_category", formData, {
          ...
          <div className="bg-white p-6 rounded-lg shadow-lg w-96">
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <input
                ...
                value={formData?.name ?? subCategory.name}
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...prev,
                    name: e.target.value,
