...
async function handler(req: NextApiRequest, res: NextApiResponse) {
    try {
        ...
        const [rows] = await  mysqlDb.execute(`SELECT 
            ...
            FROM \`categories\` category LEFT   JOIN \`sub_categories\` sub_category ON \`category\`.id = \`sub_category\`.category_id`);
       ...
       if (!(rows as ICategoryWithSubCategoryQueries[]).length) {
               ...
               return res.status(200).json(responseStructure(true, "Successfully Fetch   All Categories And Sub  Categories", categoriesAndSubCategories));
