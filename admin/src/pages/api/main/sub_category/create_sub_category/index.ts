...
const handler = createRouter<CustomNextApiRequest, NextApiResponse>()
    ...
    .post(async (req: CustomNextApiRequest, res: NextApiResponse) => {
        ...
        try {
            ...
            const { category_id, name } = parsedData.data;
            const mysqlDb = await dbConnection;

            const [categoryExists] = await mysqlDb.execute(`SELECT id FROM \`categories\` WHERE id=?`, [parseInt(category_id)]) as RowDataPacket[];
            if ((categoryExists as ISubCategory[]).length === 0) {
                return res.status(404).json(responseStructure(false, "Category Not Found"));
            }

            const [subCategoryExists] = await mysqlDb.execute(`SELECT id FROM \`sub_categories\` WHERE category_id=? AND name=?`, [parseInt(category_id), name]) as RowDataPacket[];
            if ((subCategoryExists as ISubCategory[]).length > 0) {
                return res.status(400).json(responseStructure(false, "Sub-category already exists"));
            }
            const uploadedFile = req.uploadeFile;

            const result = await mysqlDb.execute(`INSERT INTO \`sub_categories\` (category_id, name,image,is_active) VALUES (?, ?,?,1)`, [parseInt(category_id), name,uploadedFile]) as OkPacket[];

            if (result[0].affectedRows === 0) {
                return res.status(400).json(responseStructure(false, "Something went wrong"));
            }

            return res.status(201).json(responseStructure(true, "Successfully created sub-category"));
        } catch (error) {
