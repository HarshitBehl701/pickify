...
const multerMiddleware = (req: CustomNextApiRequest, res: NextApiResponse, next: Function) => {
    ...
    upload.single("image")(req as any, res as any, async (err) => {
        try {
            ...
            const [categoryRows] = await mysqlConnection.execute(`SELECT * FROM  \`categories\` WHERE id=?`, [parseInt(id)]);
            ...
            if (category.image) {
                const imagePath = path.join(process.cwd(), `public/assets/mainAssets/main/${category.image}`);
                if (fs.existsSync(imagePath)) {
                    fs.unlinkSync(imagePath);
                }
            }           
    
            await mysqlConnection.execute(`UPDATE \`categories\` SET \`image\`=? WHERE id=?`, [imageName,parseInt(id)]);
